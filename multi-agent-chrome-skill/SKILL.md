...
```bash
bash multi-chrome-agent-workspace/launch_chrome.sh [COUNT]
```
...
### Agent 1 Tasks
1. Go to https://example1.com/contact -> fill name, email, and message from the user's task details.
...
```bash
cd multi-chrome-agent-workspace/chrome-agent-N && claude
```
...
for i in 1 2 3 4 5; do
    osascript -e "tell application \"Terminal\" to do script \"cd $(pwd)/multi-chrome-agent-workspace/chrome-agent-$i && env -u CLAUDECODE -u CLAUDE_CODE_ENTRYPOINT -u CLAUDE_AGENT_SDK_VERSION -u CLAUDE_CODE_ENABLE_SDK_FILE_CHECKPOINTING -u CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS claude --dangerously-skip-permissions -p 'You are Agent $i. Read ../chat.md, find your tasks under Agent $i, and execute them. Use Chrome DevTools MCP to browse websites, search listings, and collect data. Write your findings as a markdown table back to chat.md under the Agent $i section. Mark yourself as [WORKING] while active and [DONE] when finished.'\""
    sleep 2
done
```
...
```
Tab 1: cd multi-chrome-agent-workspace/chrome-agent-1 && claude
Tab 2: cd multi-chrome-agent-workspace/chrome-agent-2 && claude
...
```
...
```bash
cat multi-chrome-agent-workspace/chat.md
```
